...
@Data
public class MovieProductionCompanyQueryBuilder {

  private StringInputFilter filter;
  private static String NESTED_FIELD = "value.keyword";
  private static String PATH = "data.production_company";

  public MovieProductionCompanyQueryBuilder(StringInputFilter filter) {
    this.filter = filter;
  }
  ...
  public QueryBuilder buildQuery() {
    ...
    if (StringUtils.isNotEmpty(filter.getEq())) {
      boolQuery.must()
          .add(QueryBuilders.nestedQuery(
              PATH, QueryBuilders.termQuery(fullPath, filter.getEq()),
              ScoreMode.Avg));
    }
    if (StringUtils.isNotEmpty(filter.getNe())) {
      boolQuery.mustNot()
          .add(QueryBuilders.nestedQuery(
              PATH, QueryBuilders.termQuery(fullPath, filter.getNe()),
              ScoreMode.Avg));
    }
